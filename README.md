...
|---|---|---|---|
| [`build-and-package`](./plugins/swe-assistant/skills/build-and-package/SKILL.md) | Any (Contributor+ most common) | Technical Knowledge, Execution | Designing or debugging a build pipeline; deciding what goes in a package, how to version it, and how to split resources for independent release lifecycles; CI hygiene |
...
| [`stress-test-understanding`](./plugins/swe-assistant/skills/stress-test-understanding/SKILL.md) | Any | Technical Knowledge (meta), Communication | Stress-testing one's own understanding of a concept, plan, or design via Socratic dialogue — force articulation, probe assumptions, counter-cases, calibrate what's known vs. still fuzzy |
...
| Skill | Stage(s) | Pillar(s) | Triggering situation |
|---|---|---|---|
| [`agile-planning`](./plugins/swe-assistant/skills/agile-planning/SKILL.md) | Any | Execution, Communication | Estimating and planning inside an agile process — user stories, spikes, task breakdown, story points, backlog grooming, sprint capacity, roadmaps |
...
| [`on-call-shift`](./plugins/swe-assistant/skills/on-call-shift/SKILL.md) | Operator (any if on-call) | Execution, Communication | Running an on-call shift when nothing is on fire — support queue triage, the P0–P4 ladder, SLI/SLO/SLA, handoffs, tracking work in tickets, avoiding burnout |
...
| [`team-rituals`](./plugins/swe-assistant/skills/team-rituals/SKILL.md) | Any | Communication, Execution | Participating in or fixing the recurring meetings — standups as a status check, honest sprint reviews, retrospectives that actually change something |
...
| [`first-run-experience`](./plugins/swe-assistant/skills/first-run-experience/SKILL.md) | Any | Communication, Execution | Designing the first encounter with something you ship — README, quickstart, setup instructions, CLI first run, or onboarding material for a new hire |
### Leadership
...
The current 50 skills are presented as one author's worked example, not as authoritative content. Faculty are explicitly invited to disagree with specific skills, fork the repository, replace content, and build alternative implementations appropriate to their student populations.
