...
description: Use when the user is setting up, evaluating, or debugging a build pipeline, or deciding how their software is packaged for release. Triggers include "how should our build work", "my build isn't reproducible", "what goes in this package", "should we version these together", "how should we split our packages", "JAR vs wheel vs crate", or "should tests run in CI". For choosing libraries or version conflicts, route to dependency-management. For test authoring or flakiness, route to writing-tests or test-determinism. For publishing or deploying what was built, route to release-hygiene or deployment-discipline.
---
