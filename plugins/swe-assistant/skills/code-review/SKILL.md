---
name: code-review
description: Use when the user is giving a code review, submitting their own code for review, reacting to review feedback (especially feedback that feels confusing or harsh), preparing a draft/WIP PR, planning a walkthrough for a large change, or triaging a review backlog. Triggers include "I'm reviewing a PR", "asked to review", "my PR is ready for review", "got harsh review feedback", "draft PR", "code walkthrough", "I have a huge review backlog", "am I being too nitpicky", or "how do I push back on review feedback I disagree with". Covers the discipline from The Missing Readme (Ch. 7) — why reviews exist, Mode A (reviewing) vs Mode B (being reviewed), the giving priorities (correctness, security, maintainability, tests, style), tone, walkthroughs for large changes, the don't-rubber-stamp rule, and the receive side. Cross-links to growth-obstacles for the impostor feelings review reliably triggers. Skip for general engineering questions, debugging, or writing code from scratch — only for reviewing or being reviewed.
---
