...
description: Use when the user is writing a commit message, preparing a pull request, restructuring commit history before review, or asking about version-control practice. Triggers include "how should I write this commit message", "my commits are messy", "should I squash these commits", "about to open a PR", or "should I rebase or merge". Covers committing often during development, rebasing and squashing before review, issue-ID prefixes for traceability, Beams' seven rules, and structuring the PR so reviewers can follow the thinking. Do not trigger for active code reviews (route to code-review) or tactical Git command questions.
---
