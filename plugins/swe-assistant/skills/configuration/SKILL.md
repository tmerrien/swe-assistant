---
name: configuration
description: Use when the user is designing, adding, or modifying application or service configuration — choosing a format (YAML, JSON, env vars, flags), setting defaults, validating config at startup, grouping related settings, or thinking about dynamic vs static config. Triggers include "how should I configure this", "config file format", "YAML or env vars", "where should these settings live", "default values for config", "validating config", "config validation", "env var vs flag", "dotenv", "12-factor config", "should this be config or hardcoded", "config-as-code", or "I'm hand-editing config in production". Walks through the configuration discipline from The Missing Readme (Chapter 4) — boring is better, log and validate all config at startup, provide sensible defaults, group related parameters, treat config as code (versioned/reviewed/tested), keep files clean, and don't edit deployed config. Do not trigger for build configuration, for feature-flag-as-product-experiment tooling, or for code-review situations.
---
