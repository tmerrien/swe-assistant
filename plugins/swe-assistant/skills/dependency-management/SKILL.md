...
description: Use when the user is adding, updating, or managing third-party dependencies — picking a library, deciding whether to take one at all, debugging version conflicts, or designing how a project pins and scopes what it depends on. Triggers include "should I add this library", "we have a version conflict", "dependency hell", "semver", "should I pin this version", "vendoring", "shading", or license obligations from a dependency. Covers semantic versioning, transitive dependencies, deliberate declaration, pinning, narrow scoping, and circular dependencies. Do not trigger for runtime API design, code reviews, or an incident caused by a bad dependency update (route to incident-response).
---
