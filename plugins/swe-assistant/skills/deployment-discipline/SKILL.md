...
description: Use when the user is designing, automating, hardening, or debugging how built and published packages get installed onto the machines that run them. Triggers include phrases like "how should we deploy this", "our deploys are manual", "our deploy broke halfway through", "we can't roll back cleanly", "deploy A before B", "tangled deploy order", "Ansible vs Terraform", "make deploys atomic", "install alongside vs overwrite", "symlink flip", "backward compatibility across deploys", or "deploy is a human clicking buttons". Walks through the deployment phase from The Missing Readme (Ch. 8) — automating deploys, making them atomic (install-and-flip), and designing applications to deploy independently (backward/forward compatibility, no ordering dependencies). For designing the build that produces the package, route to build-and-package. For release repos or release notes, route to release-hygiene. For gradual traffic-shifting after deploy (flags/canaries/blue-green/dark launch), route to operator-playbook.
---
