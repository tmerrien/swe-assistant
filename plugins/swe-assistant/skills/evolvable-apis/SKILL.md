---
name: evolvable-apis
description: Use when the user is designing an API or changing one that already has callers — adding or removing fields and methods, worrying about breaking clients, deciding whether a change needs a new version, choosing a versioning scheme, or formalizing an interface that is currently just hand-written JSON. Triggers include "how should I design this API", "will this break clients", "is this a breaking change", "should I version this API", "v1 vs v2", "how do I deprecate this endpoint", "backward compatible API change", "OpenAPI", "protobuf", "gRPC schema", "our API has too many fields", "how do I add a required field", or "we have no API spec". Walks the evolvable-API discipline from The Missing Readme (Ch. 11) — keep the surface small, publish a machine-readable schema, keep changes compatible, and version deliberately. For database schemas and data migration, route to evolvable-data. For SemVer mechanics, route to dependency-management.
---
