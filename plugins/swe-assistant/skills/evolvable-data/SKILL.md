---
name: evolvable-data
description: Use when the user is changing how data is stored or shaped — altering a schema, running a migration, backfilling records, choosing between an explicit schema and a schemaless store, splitting a shared database, or worrying a change will break downstream consumers. Triggers include "how do I migrate this schema", "add a column safely", "how do I backfill", "should this be JSON or a real column", "another team reads our tables", or "how do I rename a column safely". Covers isolating databases, explicit schemas, automated migrations decoupled from deploys, and expand-and-contract for downstream compatibility. For service API contracts, route to evolvable-apis.
---
