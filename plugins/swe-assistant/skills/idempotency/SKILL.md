---
name: idempotency
description: Use when the user is designing an operation that may be executed more than once with the same effect — an API endpoint, message-queue handler, webhook receiver, RPC handler, or anything a client might retry or a queue might replay. Triggers include "how do I make this idempotent", "is this operation idempotent", "I'm worried about double-charges", "deduplication", "exactly-once delivery", or questions about idempotency keys and request IDs. Covers what idempotency is and isn't, HTTP method semantics, the common patterns, and the failure modes when non-idempotent operations get retried. For client-side retry mechanics, route to retry-and-backoff. Skip for read operations, code reviews, or general defensive coding.
---
