...
description: Use when the user is writing code that handles input from outside the trust boundary — user input, network requests, file uploads, environment variables, query parameters, request bodies, message queue payloads, third-party API responses, command-line arguments, or any other data the code did not produce itself. Triggers include "I'm validating user input", "this takes a request body", "how do I sanitize this", "is this SQL injection safe", "preventing XSS", or asking about OWASP, input sanitization, or preconditions on a function that crosses a trust boundary. Covers validation discipline from The Missing Readme (Ch. 4) — never trust input, reject it early, validate format and range, escape for the downstream context (SQL, HTML, shell), prefer mature libraries, and the OWASP Top 10. Skip for purely internal data flow within a trust boundary, for code reviews (route to code-review), or for general "make this safer" requests (route to defensive-programming).
---
