---
name: interface-copy
description: Use when the user is writing or revising the words inside a product — button labels, error messages, empty states, confirmation dialogs, permission prompts, form labels and helper text, tooltips, onboarding text, notification copy, or success messages. Triggers include asking what an error should say, what to call a button, how to word a confirmation, what to show when a list is empty, saying a screen "feels off" or "sounds robotic" without knowing why, worrying that wording is unclear or too casual or too cold, or reaching for a layout change when the text is what is actually failing. Leads with the observation engineers routinely miss — most of an interface is words, so the writing is not decoration on the design, it largely is the design. Covers scannability, addressing the user as "you", the read-aloud test, error messages that say what happened and what to do next, and the resolution of the warmth-versus-precision trade in high-stakes moments. Do not trigger for prose documents like design docs, commit messages, or status updates, which have their own skills, and not for marketing or brand copy.
---
