...
description: Use when the user is building or changing something a person will look at and use — a screen, form, flow, dashboard, settings page, onboarding sequence, or admin panel. Triggers include planning UI work as a phase that happens after the logic is done, asking how to lay out a screen or structure a flow, saying the design will be "polished later" or handed to a designer at the end, wondering why a working feature confuses users, or choosing an icon or visual metaphor. Sets the frame the other interface skills build on — the interface is where usability is decided, usability is the floor aesthetics sit above, and discovery is cheapest at the sketch. For contested calls like minimal versus rich, route to interface-tradeoffs. For what to emphasize, route to rationing-attention. For wording, route to interface-copy. Do not trigger for backend-only work or brand identity.
---
