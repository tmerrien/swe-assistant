...
description: Use when a user-experience or interface decision is genuinely contested and the user is choosing a direction — minimal versus rich, familiar versus distinctive, fast versus deliberately slowed, consistent with the existing design system versus differentiated from it, warm versus precise wording, or shipping the expected solution versus the surprising one. Triggers include disagreeing with a designer, product manager, or teammate about a design direction, asking "should this be simpler or more expressive", "is this too plain", "are we allowed to break the pattern here", "which way should we go on this", or presenting one side of a known design argument and looking for agreement. Rather than picking a winner, this skill states both positions, names what each buys and costs, and identifies the specific condition in the user's situation that decides between them — then asks which situation they are actually in. Carries the standing interface tensions with their deciding conditions, plus the usability floor and MAYA ceiling that bound all of them. Do not trigger for settled questions with a better-supported answer, for purely technical architecture trade-offs, or when the user simply needs to know what to emphasize.
---
