---
name: logging
description: Use when the user is adding, reviewing, or designing logging in their code — choosing log levels, structuring messages, deciding what to include or redact, or setting up logging in a service. Triggers include "what log level should I use", "is this safe to log", "how do I structure my logs", "my logs are too verbose", or "why does enabling verbose logging make this bug disappear". Covers using levels appropriately, keeping messages atomic so aggregators can correlate them, keeping logging fast, and never logging sensitive data. Do not trigger for general defensive programming (route to defensive-programming), for input validation, or for active incidents.
---
