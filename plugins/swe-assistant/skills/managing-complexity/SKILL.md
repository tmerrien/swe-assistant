...
description: Use when the user is making a structural decision about code and wondering whether it makes the system harder to understand or change — adding an abstraction, deciding whether to build flexibility for a requirement they don't have yet, choosing where module boundaries go, or noticing that a change rippled further than expected. Triggers include "should I add an abstraction here", "is this over-engineered", "YAGNI", "premature optimization", "this change touched twelve files", "everything is coupled to everything", "where should this boundary go", "should I build this to be extensible", "principle of least astonishment", or "domain-driven design". Walks the complexity framework from The Missing Readme (Ch. 11) and Ousterhout — dependency, obscurity, and inertia; complexity as something you place rather than remove; YAGNI; least astonishment; encapsulating domain knowledge. For evolving an API or a database schema specifically, route to evolvable-apis or evolvable-data.
---
