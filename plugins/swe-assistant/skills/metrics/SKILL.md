---
name: metrics
description: Use when the user is adding, choosing, or designing metrics in their code — deciding between counter, gauge, and histogram, picking what to measure, integrating a metrics library, or thinking about SLOs and instrumentation. Triggers include "should this be a counter or a gauge", "how do I measure latency", "tracking p99", "what metrics should I emit", "setting up Prometheus or OpenTelemetry", or "I want to know if this is slow in production". Covers the three metric types, what to measure, standard libraries, the cardinality and sampling traps, and where latency targets actually come from. For comparing metrics against logs and traces, route to operator-playbook. For active incidents, use the metrics rather than discussing how to write them.
---
