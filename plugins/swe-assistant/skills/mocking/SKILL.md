---
name: mocking
description: Use when the user is deciding whether or how to mock a dependency in a test — reaching for a mocking framework, debating real versus fake collaborators, writing or refactoring mocks, or noticing their mocks have become their own source of bugs. Triggers include "should I mock this", "how do I mock X", "fake vs mock", "stub vs mock", "my mocks are getting complex", or "this test has too many mocks". Covers starting with inline mocks, not writing a shared mock class until logic repeats, treating excessive mocking as a code-coupling smell, and preferring to separate I/O from computation. For broader test-writing questions, route to writing-tests. For flakiness caused by mocks, route to test-determinism.
---
