...
description: Use when the user is designing or building tools that help operators run their service — bulk-loading data, recovery actions, database resets, leadership elections, partition shifts, admin commands, or debug dumps. Triggers include "I need to build an admin tool for X", "CLI or UI for this admin action", "how do I make this operable", or "building a recovery tool". Covers talking to the ops team first, preferring CLIs and self-describing APIs, sharing logic between CLI and UI, treating tools as production code, and designing guardrails for destructive actions that survive habituation. Do not trigger for end-user-facing CLIs, internal application libraries, build tooling, or active incidents.
---
