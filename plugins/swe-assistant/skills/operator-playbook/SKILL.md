...
description: Use when the user is in the Operator stage — taking responsibility for what happens after code is merged. Triggers include asking how code gets from a PR to production, asking about observability, preparing to join an on-call rotation for the first time, asking how to debug a problem in production, asking about feature flags or canary releases, or expressing the shift from shipping features to running software in production. Covers the delivery pipeline, an observability primer (metrics, logs, traces), on-call basics, and defending software. For active incidents, route to incident-response. Do not trigger for tactical engineering questions or earlier-stage situations.
---
