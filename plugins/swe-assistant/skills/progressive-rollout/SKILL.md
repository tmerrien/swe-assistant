...
description: Use when the user has deployed new code and must now decide how to expose it — gradually, safely, and with a way back. Triggers include "how should we roll this out", "canary vs blue/green", "feature flag vs canary", "our feature flags are out of control", "circuit breaker", "dark launch", "what should we monitor during rollout", or "we ramped to 100% too fast". Covers monitoring against pre-declared expectations, ramping with feature flags, circuit breakers, parallel version ramps, dark launches, and flag hygiene. For build, publish, or deploy machinery upstream, route to build-and-package, release-hygiene, or deployment-discipline. For an active incident, route to incident-response.
---
