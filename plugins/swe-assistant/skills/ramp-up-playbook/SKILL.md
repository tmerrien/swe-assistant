...
description: Use when the user is in the Ramp-Up stage of joining a new team — past the first weeks but not yet fully productive. Triggers include asking how to learn an unfamiliar codebase, putting up first PRs and wanting feedback, planning a first solo task, prepping for early 1:1s with a new manager, asking how to keep a manager informed of progress, or expressing that they want to contribute but feel they don't know enough yet. Covers working on the existing codebase with frequent reviews, building context by reading PRs and tracing how code is built and deployed, building the manager relationship deliberately, and a sustainable status rhythm. Reinforces that ramping up is active, not passive. Skip for tactical questions or general career planning.
---
