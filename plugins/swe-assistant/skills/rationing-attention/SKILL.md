...
description: Use when the user is deciding what to emphasize and what to let recede, in any medium — a screen or dashboard where everything looks equally important, a page with competing calls to action, a log-level scheme, an alert set that pages too often, or the ordering of sections in a document. Triggers include "there's too much on this page", "how do I make this stand out", "which of these should be primary", or "the important thing is getting lost". Teaches emphasis as a fixed budget rather than something that can be added — attention is finite, contrast is zero-sum, and something must recede for anything to stand out. Applies to interfaces and equally to logs, alerts, dashboards, and documents. Do not trigger for the wording of a specific string (route to interface-copy) or for contested style debates like minimal versus rich (route to interface-tradeoffs).
---
