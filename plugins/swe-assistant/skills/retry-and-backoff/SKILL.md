...
description: Use when the user is writing or designing retry logic for calls that can fail transiently — HTTP requests, database queries, RPC calls, message-queue operations, or third-party APIs. Triggers include "how do I retry this", "should I retry on failure", "what's a good backoff strategy", "exponential backoff", or "this API call sometimes fails". Covers when to retry and when not to, exponential backoff with jitter and a hard cap, the thundering-herd problem and why naive backoff makes it worse, and failing fast and loudly on unrecoverable errors. For designing the receiving operation to be safe to retry, route to idempotency. Do not trigger for non-transient logic errors, code reviews, or active incidents.
---
