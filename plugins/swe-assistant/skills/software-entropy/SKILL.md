...
description: Use when the user is expressing frustration with the messiness, inconsistency, or chaos of a codebase they're working in — especially when the frustration is tipping into blame of past developers, previous teams, or "whoever wrote this." Triggers include "this codebase is a mess", "whoever wrote this is an idiot", "why is this so chaotic", "this code is awful", "I can't believe how bad this is", or asking why a codebase is the way it is. Reframes the situation using the software-entropy concept from The Missing Readme (Ch. 3) — messy code is a natural side effect of change (differing styles, evolving requirements and stacks, accumulated fixes), not malice or incompetence. Surfaces what helps (lint/style tools, code review, continuous refactoring). Routes to technical-debt for specific debt that needs explicit management. Skip when the user is reporting an actual bug or incident, when they're proposing specific cleanup work (that's technical-debt territory), or when they're reviewing a current PR.
---
