---
name: technical-debt
description: Use when the user is identifying, prioritizing, proposing, or trying to communicate about technical debt — work owed to fix shortcomings in existing code that the team is actively paying interest on. Triggers include "this is technical debt", "we have so much tech debt", "how do I get the team/manager to let us refactor X", "should we clean this up before adding the new feature", "is this technical debt or just code I don't like", or asking about the debt matrix or how to estimate the cost of debt. Covers the principal/interest model, Fowler's debt quadrant (deliberate vs inadvertent x reckless vs prudent), not over-using the label, paying down incrementally rather than big-bang, and a 5-step framework for proposing payoff (state factually, describe risk and cost, propose, discuss alternatives, weigh trade-offs). Skip for generic frustration about messy code (route to software-entropy), for code reviews, or for tactical engineering questions.
---
