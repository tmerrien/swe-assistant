---
name: technical-debt
description: Use when the user is identifying, prioritizing, proposing, or trying to communicate about technical debt — work owed on existing code that the team is actively paying interest on. Triggers include "this is technical debt", "we have so much tech debt", "how do I get the team or my manager to let us refactor X", "should we clean this up before adding the new feature", or "is this technical debt or just code I don't like". Covers the principal and interest model, Fowler's debt quadrant, not over-using the label, paying down incrementally rather than big-bang, and a framework for proposing payoff. Skip for generic frustration about messy code (route to software-entropy) or for code reviews.
---
