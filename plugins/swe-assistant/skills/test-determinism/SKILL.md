...
description: Use when the user has a flaky test, or is writing one that touches sources of non-determinism — time, randomness, the network, the filesystem, sockets and ports, shared state, or test order. Triggers include "my test is flaky", "intermittent failure", "passes locally but fails in CI", or "non-deterministic test". Covers fixing flakies immediately, reproducing by looping, injecting clocks for time, avoiding sleeps and timeouts, closing resources properly, binding to port zero, generating unique paths, isolating and cleaning up state, and never depending on test order. For general test-writing questions, route to writing-tests. For mocking specifically, route to mocking. Do not trigger for active production incidents.
---
