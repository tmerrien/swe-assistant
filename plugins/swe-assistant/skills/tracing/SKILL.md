---
name: tracing
description: Use when the user is adding, configuring, or debugging distributed tracing — propagating trace context across service calls, setting up OpenTelemetry / Jaeger / Honeycomb / Datadog APM, understanding spans and trace IDs, or trying to debug a multi-service issue with traces. Triggers include "how do I add tracing", "distributed tracing", "propagate trace context", "OpenTelemetry", "trace headers", "spans", "why is this slow across services", "connecting traces across requests", "instrumenting for tracing", "trace ID", "parent span", or asking what tracing is and when it's useful. Walks through the tracing discipline — the book's specific ask (verify you're propagating any required state when calling other services), basic span/trace vocabulary, OpenTelemetry as the modern standard, and when traces beat logs and metrics. For comparing tracing vs logs vs metrics (which to reach for when), route to operator-playbook. For active incidents, use the traces rather than discussing how to instrument them.
---
