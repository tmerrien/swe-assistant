---
name: writing-tests
description: Use when the user is writing tests for code they own — deciding what to test, how much to test, how to keep tests clean and maintainable, working out what test type fits a situation, or thinking about coverage targets. Triggers include phrases like "how should I test this", "writing unit tests", "what should I test", "how much test coverage do I need", "should I write a test for X", "TDD", "test-driven development", "my tests are getting messy", "what makes a good test", "are these tests good", "should QA test this", "what type of test do I need here", "integration vs unit test", "I have 60% code coverage, is that enough", "the test framework is making me write too much boilerplate". Walks through the testing discipline from The Missing Readme (Chapter 6) — taking responsibility for testing your own code, the test-type taxonomy, writing clean tests with the same care as production code, not over-testing or chasing coverage percentages, and using a risk matrix to focus effort. For mocking specifically, route to mocking. For flaky / non-deterministic tests, route to test-determinism. Do not trigger for code reviews of test PRs (route to code-review) or for testing strategy at the org/architecture level.
---
